...
d.fs <- C5imp(fit.fs, metric = "usage", pct = TRUE)
...
d.f.early <- C5imp(fit.f.early, metric = "usage", pct = TRUE)
...
dsp.Rule.f <- C5imp(fit.Rule.f, metric = "splits", pct = TRUE)
...
d.20.Ruleb.f <- C5imp(fit.20.Ruleb.f, metric = "usage", pct = TRUE)
...
d.20.f.GP <- C5imp(fit.20.f.GP, metric = "usage", pct = TRUE)
# summary(fit.20.f.GP)
