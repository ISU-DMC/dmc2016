...
grid.step.3 = expand.grid(  RSAMPLE = seq(0.3, 0.8, by=0.1),
                            CSAMPLE = seq(0.3, 0.8, by=0.1))
#out.name = "final_all"
###########################################################
...
{
  r_sample = grid.step.3$RSAMPLE[i]
  c_sample = grid.step.3$CSAMPLE[i]
  xgb_params = list(
  ...
  write.csv(xg_auc3, paste0("./tuning/xgboost_step_3_", i, ".csv"), row.names = FALSE, quote = FALSE)
